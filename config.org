...
(setq undo-limit 80000000                         ; Raise undo-limit to 80Mb
      ...
      scroll-margin 0
      display-time-default-load-average nil       ; Load average isn't useful in the modeline clock
...
New frames default to a somewhat arbitrary size; pick something more
reasonable.
...
Window rotation lives under =SPC w r= and =SPC w R=, but /layout/ rotation is
handy too. The =rotate= package adds =rotate-layout= for this; bind it to
=SPC w SPC=, and fill in the missing arrow-key variants of window
navigation/swapping.
...
~hippie-expand~ is a nicer swap-in upgrade for ~dabbrev-expand~.

#+begin_src emacs-lisp
(global-set-key [remap dabbrev-expand] #'hippie-expand)
#+end_src

By default it tries, in order: file names, abbreviations, list expansion,
previous lines, dabbrev (this buffer, then all buffers, then kill ring), and
elisp symbols. ~try-expand-line~ tends to fire when a dabbrev completion is
actually wanted, so push it down; prioritise the dabbrev-family expanders
instead, including ~try-expand-dabbrev-visible~ which isn't tried by default.
...
(python +cython +lsp +pyright +tree-sitter)            ; beautiful is better than ugly
...
#+begin_src emacs-lisp
(setq doom-font (font-spec :family "IosevkaTerm Nerd Font" :size 12)
...
(defvar required-fonts '("IosevkaTerm Nerd Font" "Symbola" "Alegreya" "IBM Plex Mono"))
...
(defun projectile-ignored-project-function (filepath)
  ...
  (cl-some (lambda (p) (string-prefix-p p filepath)) projectile-ignored-projects))
#+end_src
...
Inline blame annotation on the current line, VS Code GitLens-style
(complements the fringe diffs from =vc-gutter +diff-hl=):
...
(use-package! blamer
  ...
  :init
  (map! :leader :desc "Toggle inline git blame" "t B" #'blamer-mode))
#+end_src

** GPTel

Use Gemini as =gptel= backend:
...
Run the =Claude Code= CLI in a vterm popup:
...
(use-package! claude-code
  :init
  (map! :leader :desc "Claude Code" "o c" #'claude-code-toggle)
  :config
...
Show the enclosing function/class in a posframe while scrolling, like
=nvim-treesitter-context=. Not on MELPA; pull from upstream directly.

#+begin_src emacs-lisp :tangle packages.el
(package! treesitter-context :recipe
  (:host github
...
Compile without prompting to save first, keep the compilation buffer around,
and pass =-shell-escape= (needed for packages like =minted= and
=tikz-externalize=; only enable this if you trust the =.tex= files you compile,
since it lets LaTeX run arbitrary shell commands).
...
#+begin_src emacs-lisp :tangle packages.el
;; Disabled: upstream typst-ts-compile.el has a stray ;;;###autoload cookie
;; on a `define-compilation-mode' form (a macro, not a function), which
;; loaddefs-generate copies verbatim into the autoloads file. On Emacs 31
;; that file is loaded before `compile' is required, so the macro is void
;; and `doom sync' fails. Re-enable once upstream fixes it or the recipe
;; points at a patched fork.
(package! typst-ts-mode :disable t :recipe
  (:type git
   :host sourcehut
...
lsp-mode ships a =tinymist= client (=lsp-typst.el=); nothing triggers it yet:
...
Highlight the =julia>= REPL prompt when writing REPL examples in =julia-mode=
buffers:
...
=rainbow-delimiters= is nice to have here, and =lsp-mode= is known to choke on
folding responses from the Julia language server ([[https://github.com/non-Jedi/lsp-julia/issues/35][lsp-julia#35]]); cap the
folding range as a workaround.
...
No lsp-mode client ships for uiua; register one against the =uiua lsp=
subcommand, mirroring the empty-config server entry in the nvim setup:
...
(after! lsp-mode
  (lsp-register-client
   (make-lsp-client
    ...
    :major-modes '(uiua-ts-mode)
    :server-id 'uiua-lsp)))
(add-hook 'uiua-ts-mode-hook #'lsp-deferred)
#+end_src

** Python

Use =basedpyright= instead of vanilla =pyright=:
...
Edit =.vue= single-file components with =web-mode=; lsp-mode's bundled Volar
client (=vue-semantic-server=) auto-activates for it.
...
Use built-in =asm-mode=; lsp-mode's bundled =asm-lsp= client activates on it.
